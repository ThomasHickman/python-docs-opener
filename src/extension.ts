...
import {getFunctionAtPosition, getDocWebPageFromSymbol} from "./internals";
...
export function activate(context: vscode.ExtensionContext) {
	...
	let disposable = vscode.commands.registerCommand('python-help-fetcher.getDocsOfSymbol', async () => {
		...
		if (editor) {
			...
			const symbolAtPosition = await getFunctionAtPosition(document.fileName, selection.active.line + 1, selection.active.character + 1);
